...
from snake_sim.snakes.strategies.manual_strategy import ManualStrategy
...
class SnakeStrategyFactory:
    ...
    def create_strategy(self, strategy_config: StrategyConfig) -> ISnakeStrategy:
        try:
            strat_class = self._strategy_classes[strategy_config.type]
        except KeyError:
    ...
    def create_strategies(self, config: SnakeConfig) -> Dict[int, ISnakeStrategy]:
        ...
        for priority, strat_config in strategies_dict.items():
            strategies[priority] = self.create_strategy(strat_config)
        return strategies