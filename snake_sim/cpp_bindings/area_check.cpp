...
public:
    ...
    AreaCheckResult search_best(int snake_length, bool food_check){
        ...
        while(!search_stack.empty()){
            ...
            // for (auto& dead_end : step_data->dead_end_nodes){
            //     std::cout << "(" << dead_end->id << ", (" << dead_end->start_coord.x << ", " << dead_end->start_coord.y << ")) ";
            // }
            // std::cout << std::endl;
            ...
            //     std::cout << "(" << loop.first->id << ", " << loop.second->nodes.front()->id << ") ";
            // }
            // std::cout << std::endl;
            if (forward){
                step_data->visited = true;
                if(current_node->has_tail && ! food_check){
            ...
            if (step_data->has_opening){
                ...
                if (best_needed_steps_node < best_needed_steps_here){
                    ...
                    for (auto node_with_opening : step_data->nodes_with_openings){
                        ...
                        all_food = curr_food + opening_search_data->food_to_best_opening;
                ...
                margin = total_steps - (snake_length + 1);
            }

            if (food_check){
                if (margin > result.margin || (all_food > result.food_count && margin >= 0)){
            ...
            if (!food_check && result.margin >= all_food){
                break;
            }
            ...
            if (!step_data->to_explore_nodes.empty()){
                ...
                std::vector<AreaNode*> discards;
                discards.push_back(next_node);
                if (prev_step != nullptr){
                    discards.push_back(prev_step->node);
                }
                auto valid_loops = get_valid_loop_backs(discards, step_data);
                int extra_tiles = 0;
                int extra_food = 0;
                for (auto loop_back : valid_loops){
                    extra_tiles += loop_back->tile_count;
                    extra_food += loop_back->food_count;
                }
                if (search_stack.size() >= 2){
                    ...
                    */
                    if (std::abs(delta_x) == 1 && std::abs(delta_y) == 1){
                        ...
                        total_food_count_stack.push_back(total_food_count_here + extra_food);
                        total_tile_count_stack.push_back(total_tile_count_here + extra_tiles);
                    }
                }
                else{
                    total_food_count_stack.push_back(total_food_count_here + extra_food);
    ...
    AreaCheckResult _area_check2(
        ...
        while(!nodes_to_explore.empty()){
            ...
            if (result.tile_count == 0){
                ...
                if (current_node->tile_count < 4 && current_node->id != 0){ // a node cant really have 2 or 3 tiles, next step after 1 is 4, but anyways...
